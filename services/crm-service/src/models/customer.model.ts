...
export class CustomerModel {
   ...
   */
  static async generateCustomerNumber(tenantId: string): Promise<string> {
   ...
   * Find customer by ID
   */
  static async findById(id: string, tenantId: string): Promise<Customer> {
    const result = await query(
      `SELECT * FROM customers
   ...
   */
  static async findByCustomerNumber(customerNumber: string, tenantId: string): Promise<Customer | null> {
    const result = await query(
      `SELECT * FROM customers
       WHERE customer_number = $1 AND tenant_id = $2 AND deleted_at IS NULL`,
   ...
   * Create new customer
   */
  static async create(data: Partial<Customer>, tenantId: string, userId: string): Promise<Customer> {
    ...
    if (existing) {
      throw new ConflictError(`Customer with number ${customerNumber} already exists`);
    }
   ...
   * Update customer
   */
  static async update(id: string, tenantId: string, data: Partial<Customer>): Promise<Customer> {
    ...
    updateFields.forEach((field) => {
      if (data[field as keyof Customer] !== undefined) {
        ...
        params.push(data[field as keyof Customer]);
        paramIndex++;
      }
    });
   ...
   */
  private static async validateParentCustomer(
    parentId: string,
    tenantId: string,
    ...
    if (currentCustomerId && parentId === currentCustomerId) {
      throw new ConflictError('Customer cannot be its own parent');
    }

    // Check for circular reference (walk up the tree)
    if (currentCustomerId) {
      let current = parentId;
      const visited = new Set<string>([currentCustomerId]);
      let depth = 0;
      const MAX_DEPTH = 3;

      while (current) {
        if (visited.has(current)) {
          throw new ConflictError('Circular parent relationship detected');
        }
        visited.add(current);
        depth++;

        if (depth > MAX_DEPTH) {
          throw new ConflictError(`Customer hierarchy cannot exceed ${MAX_DEPTH} levels`);
        }

        const parent = await query(
          'SELECT parent_customer_id FROM customers WHERE id = $1 AND tenant_id = $2 AND deleted_at IS NULL',
          [current, tenantId]
        );
   ...
   */
  static async getChildren(customerId: string, tenantId: string): Promise<Customer[]> {
    const result = await query(
      `SELECT * FROM customers
