...
export function EditContactPage() {
  ...
  // Fetch customer and contact data
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch customer details for breadcrumb
        ...
        const contactResponse = await api.get(`/customers/${customerId}/contacts/${contactId}`);
        const contact: Contact = contactResponse.data.data || contactResponse.data;
        ...
        setFormData({
          ...
          email: contact.email || '',
          phone: contact.phone_office || '',
          jobTitle: contact.title || '',
          isPrimary: contact.is_primary,
          isBilling: contact.is_billing,
          isTechnical: contact.is_technical,
          notes: contact.notes || '',
        });
      } catch (err: any) {
        console.error('Failed to fetch contact:', err);
        setError(err.response?.data?.message || 'Fehler beim Laden des Kontakts');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const backendData = {
        ...
        phone_office: formData.phone || null,
      ...
      await api.put(`/customers/${customerId}/contacts/${contactId}`, backendData);
      ...
      setError(err.response?.data?.message || 'Fehler beim Aktualisieren des Kontakts. Bitte versuchen Sie es erneut.');
      setIsSaving(false);
    }
  };
        ...
        <div className="flex items-center justify-center h-screen">
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-md">
            <div className="text-center">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">Kontakt aktualisiert!</h3>
